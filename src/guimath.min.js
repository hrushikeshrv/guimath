/*! guimath 2024-08-25 | (C) Hrushikesh Vaidya (@hrushikeshrv) | MIT License */
class Expression {
    constructor(t = 0) {
        (this.components = []), (this.nestingDepth = t);
    }
    add(t, a = this.components.length) {
        this.components.splice(a, 0, t);
    }
    remove(t = this.components.length - 1) {
        this.components.splice(t, 1);
    }
    toLatex() {
        let t = '';
        for (var a of this.components) t += a.toLatex() + ' ';
        return t.trim();
    }
}
class Block {
    ...
    toLatex() {
        ...
        for (var a of this.children)
            'string' == typeof a ? (t += a) : (t += a.toLatex() + ' ');
        return t.trim();
    }
    addChild(t, a = this.children.length) {
        this.children.splice(a, 0, t);
    }
    removeChild(t = this.children.length - 1) {
        this.children.splice(t, 1);
    }
}
class Component {
    constructor(t = [], a = null) {
        (this.blocks = t), (this.parent = a);
    }
    toLatex() {
        return '';
    }
    addBlock(t, a) {
        this.blocks.splice(a, 0, t);
    }
    removeBlock(t) {
        this.blocks.splice(t, 1);
    }
    isEmpty() {
        for (var t of this.blocks) if (t.children.length) return !1;
        return !0;
    }
}
...
class TwoBlockComponent extends Component {
    constructor(t) {
        let a = new Block(),
            e = new Block();
        super([a, e], t), (a.parent = this), (e.parent = this);
    }
}
...
class Cursor {
    ...
    addText(t) {
        if (null === this.block) {
            const a = new TextComponent(this.block);
            a.blocks[0].addChild(t),
                this.expression.add(a, Math.ceil(this.position)),
                (this.child = -0.5),
                this.position++;
        } else {
            const e = new TextComponent(this.block);
            e.blocks[0].addChild(t),
                this.block.addChild(e, Math.ceil(this.child)),
                this.child++;
        }
    }
    addComponent(t) {
        null === this.block
            ? (this.expression.add(t, Math.ceil(this.position)),
              (this.position = Math.ceil(this.position)),
              t instanceof GuiMathSymbol || t instanceof TextComponent
                  ? ((this.block = null),
                    (this.component = null),
                    (this.position += 0.5))
                  : ((this.block = t.blocks[0]), (this.component = t)),
              (this.child = -0.5))
            : (this.block.addChild(t, Math.ceil(this.child)),
              t instanceof GuiMathSymbol || t instanceof TextComponent
                  ? (this.child += 1)
                  : ((this.component = t),
                    (this.block = t.blocks[0]),
                    (this.child = -0.5)));
    }
    removeComponent() {
        if (null === this.block) {
            var t = this.expression.components[Math.floor(this.position)];
            (t instanceof TextComponent || t instanceof GuiMathSymbol) &&
                ((this.position = Math.floor(this.position)),
                (this.component = t),
                (this.block = t.blocks[0]),
                (this.child = -0.5),
                this.removeComponent());
        } else if (null === this.component.parent) {
            for (let t = 0; t < this.expression.components.length; t++)
                if (this.expression.components[t] === this.component) {
                    this.expression.remove(t);
                    break;
                }
            (this.position -= 0.5),
                (this.component = null),
            ...
            let a = this.component.parent;
            for (let t = 0; t < a.children.length; t++)
                if (a.children[t] === this.component) {
                    a.removeChild(t), (this.child = t - 0.5);
                    break;
                }
            (this.block = a), (this.component = a.parent);
        }
    }
    keyPress(t) {
        var a;
        characters.has(t.key)
            ? this.addText(t.key)
            : 'ArrowLeft' === t.key
            ? this.seekLeft()
            : 'ArrowRight' === t.key
            ...
            : ' ' === t.key
            ? ((a = new GuiMathSymbol(this.block, '\\:\\:')),
              this.addComponent(a))
            : '\\' === t.key
            ? ((a = new GuiMathSymbol(this.block, '\\backslash')),
              this.addComponent(a))
            : ['$', '#', '%', '&', '_', '{', '}'].includes(t.key) &&
              ((t = new GuiMathSymbol(this.block, `\\${t.key}`)),
              this.addComponent(t)),
            this.updateDisplay();
    }
    seekRight() {
        var t,
            a = this.expression.components.length - 0.5;
        this.position >= a ||
            (null === this.block
                ? ((this.position += 0.5),
                ...
                ? (t = this.component.blocks.indexOf(this.block)) ===
                  this.component.blocks.length - 1
                    ? null === this.component.parent
                        ? ((this.component = null),
                          (this.block = null),
                          (this.child = -0.5),
                          (this.position += 0.5))
                    ...
                    : ((this.block = this.component.blocks[t + 1]),
                      (this.child = -0.5))
                : (t = this.block.children[Math.ceil(this.child)]) instanceof
                      TextComponent || t instanceof GuiMathSymbol
                ? this.child++
                : ((this.component = t),
                  (this.block = this.component.blocks[0]),
                  (this.child = -0.5)));
                ...
                ? ((this.position -= 0.5),
                  ...
                  this.expression.components[this.position] instanceof
                      ...
                      : ((this.component =
                        ...
                        (this.child = this.block.children.length - 0.5)))
                : -0.5 === this.child
                ? 0 === (t = this.component.blocks.indexOf(this.block))
                    ? null === this.component.parent
                        ? ((this.component = null),
                          (this.block = null),
                          (this.child = -0.5),
                    ...
                    : ((this.block = this.component.blocks[t - 1]),
                      (this.child = this.block.children.length - 0.5))
                : (t = this.block.children[Math.floor(this.child)]) instanceof
                      TextComponent || t instanceof GuiMathSymbol
                ? this.child--
                : ((this.component = t),
                  (this.block =
    ...
    backspace() {
        var t;
        0 !== this.expression.components.length &&
            -0.5 !== this.position &&
            (null === this.block
                ? (t =
                      this.expression.components[
                          Math.floor(this.position)
                      ]) instanceof TextComponent || t instanceof GuiMathSymbol
                    ? this.removeComponent()
                    : ((this.component = t),
                      (this.block =
                          this.component.blocks[
    ...
    toLatex() {
        var t = this.expression.toLatex();
        return (this.latex = t);
    }
    toDisplayLatex() {
        let t = new TextComponent(this.block);
        t.blocks[0].addChild('|');
        let a = new FrameBox(this.block);
        null === this.block
            ? this.expression.add(t, Math.ceil(this.position))
            : ((e = this.component.blocks.indexOf(this.block)),
              this.component.removeBlock(e),
              this.component.addBlock(a, e),
              (a.blocks[0] = this.block),
              this.block.addChild(t, Math.ceil(this.child)));
        var e,
            s = this.toLatex();
        return (
            null === this.block
                ...
                : ((e = this.component.blocks.indexOf(a)),
                  this.component.removeBlock(e),
                  this.component.addBlock(this.block, e),
                  this.block.removeChild(Math.ceil(this.child))),
            s
        );
    }
    updateDisplay() {
        (this.display instanceof String || 'string' == typeof this.display) &&
...
class GuiMath {
    constructor(t, a = function (t, a) {}, e = {}) {
        (this.selector = t),
            ...
            this.elements.forEach(t => {
                t.addEventListener('click', this.showUI);
            }),
            document.addEventListener('keydown', t => {
                'false' !== this.editorWindow.dataset.visible &&
                    (MathJax.typesetClear([this.eqnDisplay]),
                    this.cursor.keyPress(t),
        ...
        const s = this.editorWindow.querySelectorAll(
                '.guimath-operator, .guimath-greek-letter',
            ),
            i = this.editorWindow.querySelectorAll('.guimath-function');
        s.forEach(a => {
            a.addEventListener('click', () => {
                var t;
                a.dataset.latexData in symbolLatexMap &&
                    ((t = new GuiMathSymbol(
                        this.cursor.block,
                        symbolLatexMap[a.dataset.latexData],
                    )),
                    this.cursor.addComponent(t),
            ...
            i.forEach(a => {
                a.addEventListener('click', () => {
                    let t;
                    'null' !== a.dataset.templateType
                        ? 'three' === a.dataset.templateType
                            ? (t = new TemplateThreeBlockComponent(
                                  this.cursor.block,
                                  a.dataset.latexData,
                              ))
                            : 'trigonometric' === a.dataset.templateType &&
                              (t = new TrigonometricTwoBlockComponent(
                                  this.cursor.block,
                                  a.dataset.latexData,
                              ))
                        : (t = new functionComponentMap[a.dataset.functionId](
                              this.cursor.block,
                          )),
                        this.cursor.addComponent(t),
                        this.cursor.updateDisplay();
    ...
    constructUI() {
        ...
        t.classList.add('_guimath_editor_window'),
            ...
            'dark' === this.options.theme?.toLowerCase().trim() &&
                t.classList.add('_guimath_dark_theme'),
            (this.editorWindow = t),
            (this.eqnDisplay = t.querySelector('._guimath_editor_display')),
            (this.eqnDisplay.innerHTML = `${this.mathDelimiter} | ${this.mathDelimiter}`),
            (this.pseudoMobileKeyboard = t.querySelector(
                '.guimath-pseudo-mobile-keyboard',
            ));
        const a = t.querySelectorAll('.guimath_tab_container'),
            e = t.querySelectorAll('.guimath_tab'),
            s = t.querySelector('.leftArrowButton'),
            i = t.querySelector('.rightArrowButton');
        s.addEventListener('click', () => {
            this.cursor.seekLeft(), this.cursor.updateDisplay();
        }),
            i.addEventListener('click', () => {
                this.cursor.seekRight(), this.cursor.updateDisplay();
            }),
            a.forEach(a => {
                a.addEventListener('click', function () {
                    e.forEach(t => {
                        t.dataset.tab === a.dataset.tab
                            ? (t.style.display = 'flex')
                            : t.removeAttribute('style');
                    });
                });
            }),
            a[0].classList.add('_guimath_active_tab'),
            a.forEach(t => {
                t.addEventListener('click', () => {
                    a.forEach(t => {
                        t.classList.remove('_guimath_active_tab');
                    }),
                        t.classList.add('_guimath_active_tab');
                });
            });
        const n = t.querySelector('.guimath_close_button_svg');
        n.addEventListener('click', this.hideUI);
        const o = t.querySelector('._guimath_clear_equation');
        o.addEventListener('click', () => {
            this.clearEquation();
        });
        const l = t.querySelector('._guimath_save_equation');
        l.addEventListener('click', () => {
            this.successCallback(this.getLatex(), this),
                this.hideUI(),
                this.isPersistent || this.clearEquation();
        }),
    ...
    rebindListeners() {
        this.elements.forEach(t => {
            t.removeEventListener('click', this.showUI);
        }),
    ...
    registerFunction(t, a, e = '', s = !1) {
        const i = document.createElement('span');
        i.classList.add('guimath-btn', 'guimath-function'),
            (i.title = e),
            (i.dataset.templateType = 'user-defined'),
            (i.dataset.functionId = 'user-defined'),
            (i.innerHTML = a),
            this.editorWindow
                .querySelector('._guimath_functions_tab')
                .appendChild(i),
            s && MathJax.typesetPromise([i]).then(() => {}),
            i.addEventListener('click', () => {
                this.cursor.addComponent(new t()), this.cursor.updateDisplay();
            });
    }
    registerSymbol(a, t, e = '', s = !1) {
        const i = document.createElement('span');
        i.classList.add('guimath-btn', 'guimath-symbol'),
            (i.title = e),
            (i.dataset.latexData = a),
            (i.innerHTML = t),
            this.editorWindow
            ...
            s && MathJax.typesetPromise([i]).then(() => {}),
            i.addEventListener('click', () => {
                var t = new GuiMathSymbol(this.cursor.block, a);
                this.cursor.addComponent(t), this.cursor.updateDisplay();
            });
    }
    static createEquationInput(t, a = {}) {
        void 0 === a.isPersistent && (a.isPersistent = !0);
        var s = document.querySelectorAll(t);
        for (let t = 0; t < s.length; t++) {
            let e = s[t];
            (e.style.display = 'none'), (e.value = '');
            const i = document.createElement('div');
            i.classList.add('_guimath_equation_input_wrapper'),
                (i.innerHTML =
            ...
            const n = i.querySelector('._guimath_equation_input'),
                o = i.querySelector('._guimath_insert_equation_button'),
                l = i.querySelector('._guimath_equation_input_preview');
            o.id = `_guimath_insert_equation_button_${t}`;
            const r = new GuiMath(
                `#_guimath_insert_equation_button_${t}`,
                function () {},
                a,
            );
            r.successCallback = function (t, a) {
                0 < t.length
                    ? ((e.value = t),
                      MathJax.typesetClear([l]),
                      (l.innerHTML = `$ ${t} $`),
                      MathJax.typesetPromise([l]).then(() => {}),
                      (o.textContent = 'Edit'))
                    : ((e.value = ''),
                      MathJax.typesetClear([l]),
                      (l.innerHTML = ''),
                      (o.textContent = 'Add Equation')),
                    e.validity.valid
                        ? (n.classList.remove(
                              '_guimath_equation_input_invalid',
                          ),
                          n.classList.add('_guimath_equation_input_valid'))
                        : (n.classList.add('_guimath_equation_input_invalid'),
                          n.classList.remove('_guimath_equation_input_valid'));
            };
        }
    }
}
